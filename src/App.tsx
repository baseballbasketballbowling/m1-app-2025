...
import { 
  ...
  TrendingUp, Award, Sparkles, Info
} from 'lucide-react';
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm text-left">
                  ...
                  <tbody className="divide-y divide-slate-800">
                    {ranking.map((item, i) => { 
                      const isRevealed = displayData.revealedStatus?.[item.c.id];
                      const myScoreVal = scores[item.c.id]?.[user?.name || ''];
                      const officialScore = displayData.officialScores?.[item.c.id];
                      return (
            ...
            <div className="bg-gradient-to-br from-slate-900 to-slate-800 p-6 rounded-xl border border-blue-500/30 shadow-lg relative overflow-hidden">
                ...
                <div className="space-y-4 text-sm text-slate-300 relative z-10">
                    <div className="flex gap-3">
                        <div className="bg-blue-500/20 p-2 rounded h-fit"><Crown size={18} className="text-blue-400"/></div>
                        <div>
                            <span className="font-bold text-white block mb-1">1. 3連単予想をしよう！</span>
                            このページで優勝〜3位までのコンビを予想しよう。右上のメニューから「みんなの予想」も見れるよ！
                        </div>
                    </div>
                    <div className="flex gap-3">
                        <div className="bg-red-500/20 p-2 rounded h-fit"><Mic size={18} className="text-red-400"/></div>
                        <div>
                            <span className="font-bold text-white block mb-1">2. 当日はリアルタイム採点！</span>
                            ネタ終了後に採点タイム！みんなの点数を見ながら決勝進出者を予想してドキドキを楽しもう。
                        </div>
                    </div>
                    <div className="flex gap-3">
                        <div className="bg-yellow-500/20 p-2 rounded h-fit"><Vote size={18} className="text-yellow-400"/></div>
                        <div>
                            <span className="font-bold text-white block mb-1">3. 最終決戦も投票！</span>
                            勝ち残った3組から優勝者を決めるのはあなた！最後まで参加してね。
                        </div>
                    </div>
                </div>
            </div>
      ...
      {showResetModal && (
        <div className="fixed inset-0 z-[60] bg-black/80 flex items-center justify-center p-4">
          <div className="bg-slate-900 w-full max-w-sm rounded-xl border border-slate-700 p-6 space-y-4">
            ...
            <div className="space-y-3">
                <button onClick={() => executeDatabaseReset('predictions_only')} className="w-full py-3 bg-blue-600/30 border border-blue-700 text-blue-300 rounded-lg font-bold hover:bg-blue-600/50 transition-colors"><span className="whitespace-nowrap">予想データのみリセット</span><p className='font-normal text-xs mt-1 text-slate-400'>(採点結果、ユーザー情報は保持)</p></button>
                <button onClick={() => executeDatabaseReset('scores_only')} className="w-full py-3 bg-orange-600/30 border border-orange-700 text-orange-300 rounded-lg font-bold hover:bg-orange-600/50 transition-colors"><span className="whitespace-nowrap">採点データのみリセット</span><p className='font-normal text-xs mt-1 text-slate-400'>(予想、ユーザー情報は保持)</p></button>
                 <button onClick={() => executeDatabaseReset('all')} className="w-full py-3 bg-red-600/30 border border-red-700 text-red-300 rounded-lg font-bold hover:bg-red-600/50 transition-colors"><span className="whitespace-nowrap">全データ（ユーザー認証情報含む）リセット</span><p className='font-normal text-xs mt-1 text-slate-400'>(新規ユーザー登録から必要)</p></button>
            </div>
      ...
      {showNicknameModal && (
        <div className="fixed inset-0 z-[60] bg-black/80 flex items-center justify-center p-4">
          <div className="bg-slate-900 w-full max-w-sm rounded-xl border border-slate-700 p-6 space-y-4">
            ...
            <input type="text" value={newNickname} onChange={e => setNewNickname(e.target.value)} className="w-full bg-slate-800 border border-slate-700 rounded p-3 text-white focus:ring-2 focus:ring-yellow-500 outline-none" placeholder="新しいニックネーム" />
